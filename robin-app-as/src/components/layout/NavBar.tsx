    ...
    <header className="relative z-50 w-full px-4 sm:px-6 lg:px-10 xl:px-20">
      <div className="mx-auto flex h-20 max-w-[1600px] items-center justify-between">
        <Link href="/" aria-label="Robin App AS home">
          <img src="/robinLogo.png" alt="Robin App AS Logo" className="h-12 w-auto sm:h-14" />
        </Link>
      ...
      {isOpen && (
        <nav id="mobile-menu" className="absolute left-4 right-4 top-20 rounded-xl border border-gray-700 bg-[#0a0a0a] p-5 shadow-xl lg:hidden">
          <ul className="flex flex-col gap-1 text-lg font-bold">
            ...
            <li className="mt-3">
              <Link href="/contact" onClick={() => setIsOpen(false)} className="block rounded-lg bg-[#42a9b8] px-4 py-3 text-center text-white">Get in Touch →</Link>
            </li>
          </ul>
        </nav>
      )}
